 ...
 * Author: Gabrielle
 *
 * Created on July 9, 2020, 1:36 AM
 */
...
/*
 * save mode, added struct element display; updated board display and brdFile
 * 
 * to be fixed/added: stats tracking, cin.fail, add struct game?
...
bool reRun();
void rdFile(Cell **game, int &rows, int &cols);
...
void save(Cell **board, int row, int col)
{
    ...
    //{
      //  for (int j = 0; j < col; j++) 
        //{ 
            //progrss.write(reinterpret_cast<char *>(&board),(row*col)*sizeof(Cell));
        //}
    //}
    for (int i = 0; i < row; i++) 
    {
